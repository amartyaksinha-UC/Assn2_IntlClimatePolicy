...
# Merge the 'ave_temp' dataframe with 'avg_by_county'
...
# Plot the figure with the regression line
...
# Plot the figure with the regression line for hotter days
...
## 5. a)
```{r}
# Plot the figure with a regression line for each county
ggplot(merged_df, aes(x = hotterdays, y = cruderate, color = county.x)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("Relationship between Hotter Days and Over-65 Mortality Rates by County") +
  xlab("Hotter Days") +
  ylab("Over-65 Mortality Rates") +
  theme(legend.position = "bottom")
```
