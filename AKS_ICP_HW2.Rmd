...
ggplot(avg_by_county, aes(x = hotdays, y = cruderate)) +
  ...
  xlab("Hot Days") +
  ylab("Over-65 Mortality Rates") +
  annotate("text", x = min(avg_by_county$hotdays), y = max(avg_by_county$cruderate), label = paste("Slope =", round(slope_hotterdays, 2)), hjust = 0, vjust = 3) +
...
subset <- merged_df[merged_df$county.x %in% selected_counties, ]
